...
(function () {
  ...
  // ====== CSS/* public/sd-chat.js — bubble + UI + invii “silenti” + link cliccabili */
(function () {
  // ====== CONFIG ======
  const ENDPOINT = 'https://assistant-api-xi.vercel.app/api/assistant';
  const CTA_URL = 'https://www.suitedigitale.it/candidatura/';
  ...
  /* Link SEMPRE bianchi e cliccabili dentro i messaggi */
  ...
  async function ask(prompt, opts={silent:false, meta:null}) {
    ...
    // Prompt “specialista” + MarkSelling + KPI realistici
    const CONTEXT = `
...
Ricorda i benefici e la USP: **MarkSelling™**, il metodo che unisce marketing & vendite in **un unico team operativo** (strategist, media buyer, venditori),
con piattaforma all-in-one (funnel, CRM, automazioni, lead gen, presa appuntamenti). 
Se la domanda non è pertinente al marketing/servizi Suite Digitale, spiega che sei specializzato e invita a scriverci a **marketing@suitedigitale.it** o **+39 351 509 4722**.

Se l’utente chiede se i KPI del simulatore sono realistici, rispondi con sicurezza che **SÌ**:
sono **basati su migliaia di dati** aggiornati da **220+ progetti reali** che gestiamo ogni mese.

Quando citi KPI numerici: **ROI** usa il simbolo **%** (es. -92,59%) e **ROAS** usa il formato **0,4x**.

Nelle risposte usa **grassetto** per concetti chiave, elenco numerato o puntato se serve.
Chiudi sempre con un invito alla consulenza:
...
- perché è importante un team integrato marketing+vendite (MarkSelling™);
- se ROI/ROAS fossero deboli: suggerisci la **consulenza gratuita** per rimettere in rotta;
- se i numeri fossero buoni: come si potrebbe **scalare** con strategia e controllo KPI;
